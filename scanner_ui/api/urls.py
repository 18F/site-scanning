...
from . import views
from .views import DomainsViewset, ListsViewset, ScansViewset, meta

domains_list = DomainsViewset.as_view({"get": "list"})
domains_detail = DomainsViewset.as_view({"get": "retrieve"})
scans_list = ScansViewset.as_view({"get": "list"})
scans_detail = ScansViewset.as_view({"get": "retrieve"})
scan = ScansViewset.as_view({"get": "scan"})
agencies = ListsViewset.as_view({"get": "agencies"})
...
urlpatterns = [
    ...
    path(
        "lists/<scantype>/values/<field>/<subfield>/",
        fieldvalues,
        name="subfieldvalues",
    ),
    ...
    path(
        "date/<date>/scans/<scantype>/csv/", views.retrieve_csv, name="date-scans-csv"
    ),
    ...
    path(
        "date/<date>/lists/<scantype>/domaintypes/",
        domaintypes,
        name="date-domaintypes",
    ),
    path(
        "date/<date>/lists/<scantype>/values/<field>/",
        fieldvalues,
        name="date-fieldvalues",
    ),
    path(
        "date/<date>/lists/<scantype>/values/<field>/<subfield>/",
        fieldvalues,
        name="date-subfieldvalues",
    ),
]
