...
def search200json(request):
    ...
    s = getquery(
        ...
        org=org,
        mimetype=mimetype,
        statuscodelocation=statuscodelocation,
...
def search200csv(request):
    ...
    for k, v in extram[extraindex]["mappings"]["scan"]["properties"]["data"][
        "properties"
    ].items():
        try:
            for field in v.keys():
...
def search200(request, displaytype=None):
    ...
    mimetypes = get_list_from_fields(
        indexbase + "-pagedata", "data", subfield="content_type"
    )
    ...
    if es.indices.exists(index=displaytypeindex):
        s = Search(using=es, index=displaytypeindex).filter(
            "terms", domain=pagedomainlist
        )
    else:
        s = Search(using=es, index=indexbase + "-pagedata").filter(
            "terms", domain=pagedomainlist
        )
    ...
    popups.append(
        popupbuilder("200page", my200pages, selectedvalue=periodize(my200page))
    )
    ...
    if my200page != "All Scans":
        ...
        popups.append(
            popupbuilder("mimetype", mimetypes, selectedvalue=mimetype, disabled=True)
        )
    ...
    for i in results:
        ...
        if displaytype == "200-developer":
            ...
            if i.domain in extradata and my200page != "All Scans":
                ...
                column["Content Type"] = extradata[i.domain][selectedpage][
                    "content_type"
                ]
                column["File Size (B)"] = extradata[i.domain][selectedpage][
                    "content_length"
                ]
            else:
                column["Final URL"] = ""
        ...
        # 200-codejson style display
            ...
            if i.domain in extradata and my200page != "All Scans":
                ...
                column["Code.gov Measurement Type"] = extradata[i.domain][selectedpage][
        ...
        # 200-data.json style display
            ...
            if i.domain in extradata and my200page != "All Scans":
                ...
                column["Opendata Conformity"] = extradata[i.domain][selectedpage][
        ...
        # dap style display
            ...
            popups = []
            presentlist = ["DAP Present", "DAP Not Present", "All"]
            popups.append(popupbuilder("present", presentlist, selectedvalue=present))
            popups.append(popupbuilder("date", dates, selectedvalue=date))
            popups.append(popupbuilder("agency", agencies, selectedvalue=agency))
            popups.append(
                popupbuilder("domaintype", domaintypes, selectedvalue=domaintype)
            )
            popups.append(popupbuilder("org", orgs, selectedvalue=org))
        ...
        # third_parties style display
            ...
            column["Branch"] = i.domaintype
            try:
                column["Known Services"] = ", ".join(
                    extradata[i.domain]["known_services"]
                )
                column["Unknown Service Domains"] = ", ".join(
                    extradata[i.domain]["unknown_services"]
                )
            except Exception:
        ...
        # default to the basic 200 scans search
            ...
            if displaytype is None or displaytype == "None":
                ...
                # We should never hit this, but if we do, we will still display a page
                # and the displaytype will help us debug.
...
def searchUSWDScsv(request):
    ...
    try:
        ...
        logger.error("csv error", exc_info=exc)
...
def searchUSWDS(request):
    ...
    try:
        ...
        logger.error("elasticsearch error", exc_info=exc)
        versions = []
    ...
    try:
        ...
        logger.error("query isn't a number", exc_info=exc)
        query = 0
...
def privacy(request):
    ...
    s = getquery(
        ...
        agency=agency,
        domaintype=domaintype,
        statuscodelocation="data.status_code",
    )
...
def sitemap(request):
    ...
    for i in results:
        ...
        column["Locations in robots.txt"] = " ".join(
            i.data["sitemap_locations_from_robotstxt"]
        )
        # store the column in the result, also populate the columns now, since
        # the results seem to be a type of dictionary that doesn't respond to .keys()
        columns = list(column.keys())
