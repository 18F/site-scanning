...
def getquery(
    ...
    # Present/not present may look different depending on the displaytype
    if displaytype == "dap":
    ...
    if version != "all versions" and version is not None:
        if version == "detected versions":
            ...
            s = s.query(
                "query_string", query=versionquery, fields=["data.uswdsversion"]
            )
    ...
    if mimetype is not None and page is not None and indexbase is not None:
        ...
        if mimetype != "all content_types":
            ...
            domains = domainsWith(
                page, "content_type", mimetypequery, indexbase + "-pagedata"
            )
            if len(domains) > 0:
                ...
                s = s.query(
                    ~Q("match_all")  # pylint: disable=invalid-unary-operand-type
                )
...
def get_list_from_fields(index, field, subfield=None):
    ...
    try:
        ...
        print(
            "warning:  cannot sort list with varying types from field:",
            field,
            "subfield:",
            str(subfield),
            "index:",
...
def domainsWith(page, key, value, index):
    fielddata = ["data.*.content_type", "domain"]
...
def mixpagedatain(scan, indexbase, indextype="pagedata"):
    es = Elasticsearch([os.environ["ESURL"]])
    suffix = "-" + indextype
    s = Search(using=es, index=indexbase + suffix).filter(
        "terms", domain=[scan["domain"]]
    )
    try:
        for i in s.scan():
